/*
 Securino.cpp - Security Library for Arduino.
    Version 1.0.0
  Created by  
        Rocha, IF
        Verly, PFS
...
//--------------Funcoes Públicas -------------------------------
byte* Securino::encript(String operacao, byte in[16], byte k[16] ){
  setKey(k);
  setData(in);
...
byte* Securino::decript(String operacao, byte in[16], byte k[16] ){
  setKey(k);
  setData(in);
  if(operacao=="aes-128-ecb"){
...
void Securino::encAES128ecb(byte plainText[][4], byte chave[][4]){
...
void Securino::decAES128ecb(byte plainText[][4], byte chave[][4]){
